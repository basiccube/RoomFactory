...
#macro LAYERTYPES_WINDOW_WIDTH 256
#macro LAYERTYPES_WINDOW_HEIGHT 340

#macro LAYERTYPES_LIST_WIDTH 238
#macro LAYERTYPES_LIST_HEIGHT 256
...
function ui_layertypes()
{
	...
	ImGui.SetNextWindowPos(LAYERTYPES_WINDOW_X, LAYERTYPES_WINDOW_Y, ImGuiCond.Always)
			...
			for (var i = 0, n = array_length(arr); i < n; i++)
			{
				...
				if ImGui.BeginTabItem(catName)
				{
					if ImGui.BeginListBox($"## {catName} list", LAYERTYPES_LIST_WIDTH, LAYERTYPES_LIST_HEIGHT)
					{
						...
						var func = method({ cat : cat }, function(name, index)
						{
							var sprite = cat[$ name]
							var sy = ImGui.GetCursorScreenPosY()
							if (sy > 0 && sy < LAYERTYPES_WINDOW_Y + LAYERTYPES_WINDOW_HEIGHT)
							{
								ImGui.Image(sprite.GetSprite(), 0, c_white, 1, 32, 32)
								ImGui.SameLine()
							}
							
							if ImGui.Selectable(name, false, ImGuiSelectableFlags.None, 0, 32)
							{
								print($"press {name}")
